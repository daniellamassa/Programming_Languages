...
(define the-grammar
  '((program                        ;; <Program> ::= 
    ...
    (expression                                        ;; <expression> ::=
     ...
     proc-exp)                                         ;;   Abstract  (proc-exp vars body)
    ...
    (expression                                                                    ;; <expression> ::=
     ("letrec" identifier "(" (arbno identifier) ") =" expression "in" expression) ;;  letrec <Identifier>(<Identifier>*) = <Expression> in <Expression>
     letrec-exp)                                                                   ;;  (letrec-exp p-name p-vars p-body body) 
    ...
    (expression                            ;; <Expression> ::=
     ...
     (expression                           ;; <Expression> ::=
      ...
      block-exp)                           ;;   Abstract       (block-exp expls)
    )
  )
...
(define value-of-exp
  (lambda (exp env)
    (cases expression exp
      ...
      ;;  ======== HW 6 PART 2 ==========
      [block-exp (expls) (fold-left (lambda (acc val) (value-of-exp val env)) (unit-val) expls)]
      ...
      [call-exp [exp exps]
        (let [[ev1 (value-of-exp exp env)]]
          (cases expval ev1
            [proc-val [vars body saved-env]
              (if (not (= (length vars) (length exps))) (raise-exception 'value-of-exp "unequal length: arguement names & val")
                (value-of-exp body (fold-left (lambda (acc name val) (extend-env name (value-of-exp val env) acc)) saved-env vars exps)))]
            [else
              (raise-exception
                'value-of-exp
